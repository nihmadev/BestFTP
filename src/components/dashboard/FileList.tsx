    ...
    return (
        <div
            ...
            className={`grid h-9 border-b border-card-default/30 cursor-default text-[13px] text-text-primary items-center hover:bg-subtle-secondary active:bg-subtle-tertiary transition-colors relative
                       ${isSelected ? 'bg-accent/10 hover:bg-accent/15' : ''} ${isFocused && file.name === ".." ? 'bg-subtle-secondary' : ''} ${file.name === ".." ? 'hover:bg-subtle-secondary' : ''}`}
            style={gridStyle}
...
export const FileList = React.memo(({
    ...
    const keyboardSelectedPathRef = useRef<string | null>(null);
    ...
    const handleItemMouseDown = useCallback((e: React.MouseEvent, file: FileItem) => {
        ...
        // Update focus
        focusedPathRef.current = file.full_path;
        // Reset keyboard selection when using mouse
        keyboardSelectedPathRef.current = null;
        ...
        // Trigger context menu on left click for local executables
        if (!isRemote && !file.is_directory && isExecutable(file.name) && !e.ctrlKey) {
            onFileContextMenu(e, file, isRemote);
        }
    ...
    const handleItemClick = useCallback((e: React.MouseEvent, file: FileItem) => {
        ...
        // Prevent selection for parent directory but allow click processing
        if (file.name === "..") {
            // Don't add to selection, but don't prevent the click event
            return;
        }
    ...
    const handleKeyDown = (e: React.KeyboardEvent) => {
        ...
        const updateSelection = (index: number) => {
            ...
            if (file) {
                focusedPathRef.current = file.full_path;
                keyboardSelectedPathRef.current = file.full_path;
                // Allow selection for navigation purposes, but parent directory won't be visually selected
                onSelect(new Set([file.full_path]));
                ...
                <div className="flex flex-col">
                    {sortedFiles.map((file) => (
                        <FileRow
                            ...
                            file={file}
                            isSelected={file.name !== ".." && selectedPaths.has(file.full_path)}
                            isFocused={focusedPathRef.current === file.full_path}
                            isKeyboardSelected={keyboardSelectedPathRef.current === file.full_path}
                            gridStyle={gridStyle}
