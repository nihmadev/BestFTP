...
export function useFileOperations(
    ...
    const handleConfirmDelete = useCallback(async () => {
        ...
        for (const file of files) {
            try {
                if (transferQueue && isRemote) {
                    const id = transferQueue.addToQueue(`Deleting ${file.name}`);
                    transferQueue.startTransfer(id);
                    transferQueue.currentTransferIdRef.current = id;
                    const stopSimulation = transferQueue.simulateProgress(id, 5000);

                    const result = await ftp.deleteFile(file.full_path, isRemote);
                    stopSimulation();

                    if (result.success) {
                        transferQueue.completeTransfer(id, true);
                        successCount++;
                    } else {
                        transferQueue.completeTransfer(id, false, result.error || undefined);
                        errors.push(result.error);
                    }
                    transferQueue.currentTransferIdRef.current = null;
                } else {
                    const result = await ftp.deleteFile(file.full_path, isRemote);
                    if (result.success) {
                        successCount++;
        ...
        switch (action) {
            ...
            case 'download':
                if (isRemote) {
                    ...
                    if (transferQueue) {
                        const id = transferQueue.addToQueue(file.name);
                        transferQueue.startTransfer(id);
                        transferQueue.currentTransferIdRef.current = id;
                        const stopSimulation = transferQueue.simulateProgress(id, 3000);
                        ...
                        if (res.success) {
                            ...
                            transferQueue.completeTransfer(id, false, res.error || undefined);
            ...
            case 'run':
                try {
                    const res = await ftp.runExecutable(file.full_path, isRemote);
                    if (res.success) {
                        addToast(res.data || "Started execution", 'success');
                    } else {
                        addToast(res.error || "Failed to run", 'error');
                    }
                } catch (e) {
                    addToast(String(e), 'error');
                }
